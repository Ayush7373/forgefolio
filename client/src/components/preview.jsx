...
const Preview = ({ formData, template }) => {
  ...
  // 1. Add this helper near the top of preview.jsx
  const getLayoutClasses = (layout) => {
    const settings = {
      compact: { padding: "p-6", gap: "gap-8", sectionGap: "space-y-12" },
      comfortable: { padding: "p-12", gap: "gap-12", sectionGap: "space-y-20" },
      spacious: { padding: "p-20", gap: "gap-20", sectionGap: "space-y-32" }
    };
    return settings[layout] || settings.comfortable;
  };

  // 2. Theme configuration
  const theme = formData.theme || { font: "Poppins", color: "#2563eb", layout: "comfortable" };
  const layout = getLayoutClasses(theme.layout);

  // 1. MINIMALIST FOUNDATION
  const Minimal = () => (
    <div
      className={`${layout.padding} ${layout.sectionGap} transition-all duration-500`}
    ...
    >
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-10">
        <div className="space-y-6 flex-1">
          <div className="h-1.5 w-16 rounded-full" style={{ backgroundColor: theme.color }}></div>
          ...
          <p className="text-xl md:text-2xl font-bold uppercase tracking-tight break-words" style={{ color: theme.color }}>
        ...
        <div className={layout.sectionGap}>
          {(formData.projects || []).map(p => (
            <div key={p.id} className={`grid grid-cols-1 md:grid-cols-2 ${layout.gap} items-start group`}>
              ...
              <div className="space-y-4">
                 ...
                 <ProjectLink url={p.link} className="pt-2" style={{ color: theme.color }} />
              </div>
            </div>
          ))}
        </div>
  ...
  const Aurora = () => (
    <div
      className={`${layout.padding} min-h-full bg-slate-950 text-white relative overflow-hidden flex flex-col items-center transition-all duration-500`}
      style={{ fontFamily: theme.font + ', sans-serif' }}
    >
      ...
      <div className={`relative z-10 ${layout.sectionGap}`}>
        <div className="backdrop-blur-3xl bg-white/5 border border-white/10 p-10 rounded-[3rem] space-y-8 text-left">
          ...
          <p className="text-lg md:text-xl font-medium uppercase break-words" style={{ color: theme.color }}>
        ...
        <div className={`grid grid-cols-1 md:grid-cols-2 ${layout.gap}`}>
           {(formData.projects || []).map(p => (
             <div key={p.id} className="p-8 rounded-[2.5rem] bg-white/5 border border-white/10 hover:bg-white/10 transition-all flex flex-col h-full overflow-hidden text-left">
                <h3 className="text-2xl font-black uppercase mb-3 break-words" style={{ color: theme.color }}>
                  {String(p.title || "")}
                </h3>
                ...
                <div className="mt-auto">
                  <ProjectLink url={p.link} style={{ color: theme.color }} />
                </div>
             </div>
           ))}
        </div>
  ...
  const Bento = () => (
    <div
      className={`${layout.padding} bg-gray-100 dark:bg-black min-h-full w-full transition-all duration-500`}
      style={{ fontFamily: `${theme.font}, sans-serif` }}
    >
      <div className={`grid grid-cols-1 md:grid-cols-12 ${layout.gap}`}>
        ...
        <div className="md:col-span-4 p-10 rounded-[2.5rem] flex items-center justify-center shadow-lg text-center" style={{ backgroundColor: theme.color, color: 'white' }}>
  ...
  const Cyber = () => (
    <div
      className={`${layout.padding} bg-[#050505] text-[#06b6d4] min-h-full w-full font-mono py-8 transition-all duration-500`}
      style={{ fontFamily: `${theme.font}, sans-serif` }}
    >
      <div className={`border border-cyan-500/30 p-10 rounded-2xl shadow-[inset_0_0_20px_rgba(6,182,212,0.1)] ${layout.sectionGap} text-left`}>
        <header className="border-b border-cyan-500/30 pb-8 space-y-4">
          ...
          <p className="text-lg md:text-xl tracking-[0.4em] font-bold uppercase break-words" style={{ color: theme.color }}>
            {String(formData.title || "")}
          </p>
        </header>
        <div className={`grid grid-cols-1 md:grid-cols-2 ${layout.gap} text-left`}>
  ...
  const TerminalTemplate = () => (
    <div
      className={`${layout.padding} bg-[#0d1117] text-[#c9d1d9] font-mono min-h-full w-full py-8 text-left transition-all duration-500`}
      style={{ fontFamily: `${theme.font}, sans-serif` }}
    >
      <div className={`${layout.sectionGap}`}>
        ...
        <section className="space-y-4">
          ...
          <p className="text-sm tracking-[0.2em] font-bold break-words" style={{ color: theme.color }}>
        ...
        <section className="space-y-6">
          ...
          <div className={`grid grid-cols-1 ${layout.gap}`}>
            {(formData.projects || []).map(p => (
  ...
  const CardsTemplate = () => (
    <div
      className={`${layout.padding} bg-gray-50 dark:bg-black min-h-full w-full py-8 text-left transition-all duration-500`}
      style={{ fontFamily: `${theme.font}, sans-serif` }}
    >
      <div className={`${layout.sectionGap}`}>
